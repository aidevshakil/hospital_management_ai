...
const INITIAL_DOCTORS: Doctor[] = [1, 2, 3].map((item) => ({
  id: `#DOC-${1000 + item}`,
  name: `Dr. John Doe ${item}`,
...
const STATUS_CLASS: Record<Doctor['status'], string> = {
  Active: styles.statusConfirmed,
  Inactive: styles.statusCancelled,
};

const SPECIALTIES = ['Cardiology', 'Neurology', 'Pediatrics', 'General Medicine', 'Surgery', 'Orthopedics'];
...
export default function AdminDoctors() {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    if (modalMode === 'edit' && selected) {
      ...
      const nextId = `#DOC-${1000 + doctors.length + 1}`;
        ...
        <Modal title={modalMode === 'edit' ? `Edit ${selected?.name ?? ''}` : 'Add Doctor'} onClose={closeModal}>
          <form className={styles.form} onSubmit={handleSubmit}>
            ...
            <div className={styles.formActions}>
              ...
              <button type="submit" className="btn btn-primary">{modalMode === 'edit' ? 'Save Changes' : 'Add Doctor'}</button>
            </div>
          </form>
        </Modal>
      )}
